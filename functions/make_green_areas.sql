...
-- Iterate through linestrings, assigning each to a cluster if there is an 
-- intersection, or creating a new cluster if there is none
-- XXX [jm] Only include ways with tag_id < 3000 (refer to mapconfig.xml)
FOR this_id, this_geom IN 
    SELECT id, the_geom FROM (SELECT * FROM ways WHERE tag_id < 3000) 
    LOOP
    -- Look for an intersecting cluster.  (There may be more than one.)
    SELECT cluster_id FROM green_areas 
        WHERE ST_Intersects(this_geom, green_areas.geom)
...
-- Iterate through the green_areas, combining them if they intersect
LOOP
    SELECT a.cluster_id, b.cluster_id FROM green_areas a, green_areas b 
        WHERE ST_Intersects(a.geom, b.geom) AND a.cluster_id < b.cluster_id
        INTO id_a, id_b;

    EXIT WHEN id_a IS NULL;
    -- Merge cluster A into cluster B
    UPDATE green_areas a 
        SET geom = ST_Union(a.geom, b.geom), ids = array_cat(a.ids, b.ids)
        FROM green_areas b
